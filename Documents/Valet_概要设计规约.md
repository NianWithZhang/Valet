...
系统的图片存储在服务器端Web Api程序根目录下的wwwroot文件夹内，并将此文件夹作为Web服务的目录，在文件夹内的clothespics和suitpics中分别存储衣物图片和穿搭图片，并在temp中存储压缩图片时的缓存文件。安卓手机端可以通过直接访问图片文件的URL获取到穿搭或衣物的图片，以此加强信息接口和图片数据接口的统一性。

### 2.6 系统出错处理设计

#### 2.6.1 出错信息及对应的补救措施

2.6.1.1 用户注册时两次输入的密码不同 
...
补救措施：提示用户检查网络连接，返回重新获取。



2.6.1.6 获取本地相册或打开摄像头拍照失败

出错信息：获取相册或拍照失败

补救措施：提示用户权限未开启，等待用户开启权限，返回重新获取



#### 2.6.2 系统维护设计

使用git进行版本控制，维护一条master主分支，开辟出其他3条分支，不同的开发人员独立 地在不同的分支上进行开发，有效避免多人开发导致程序版本控制的混乱，最后再merge到 master分支上。 

考虑安全性的要求，有效阻止非法嵌入等攻击行为。 保证后端服务器的安全，保证服务器所在的的物理安全，防止意外发生导致APP全面瘫 痪。定时查看并监控系统日志，及时发现系统中出现的各种问题。定期对数据库的数据进 行增量备份，将对系统来说重要的数据在不同的数据中心。即使是某地数据中心崩溃，也 可从其他地区的数据中心将数据恢复过来。 
