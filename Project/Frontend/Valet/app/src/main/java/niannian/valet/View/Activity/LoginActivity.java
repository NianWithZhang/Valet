...
//            autoLogin = savedInstanceState.getBoolean("autoLogin");
        ...
        passwordText = findViewById(R.id.confirmPasswordText);

        rememberUserSwitch = findViewById(R.id.rememberUserSwitch);
    ...
    private void login(boolean ans){
        if(!ans) {
...
//        Toast.makeText(getApplicationContext(),userIDText.getText().toString(),Toast.LENGTH_SHORT).show();
    ...
    public void tryChangeActivityButton_Click(){
        Intent intent = new Intent(getApplicationContext(), EditClothesActivity.class);
        startActivity(intent);
//        LoginActivity.this.finish();
        overridePendingTransition(android.R.anim.slide_in_left, android.R.anim.fade_out);
    }
    public void tryChangeActivityButton1_Click(){
