...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userIDText = findViewById(R.id.userIdText_register);
        passwordText = findViewById(R.id.passwordText_register);
        confirmPasswordText = findViewById(R.id.confirmPasswordText_register);
    }
    ...
    private void registerCallback(boolean status){
        if(status){
            Toast.makeText(this,"注册成功",Toast.LENGTH_SHORT).show();
            backLoginButton_Click();
        }else{
            Toast.makeText(this,"用户名已被注册",Toast.LENGTH_SHORT).show();
        }
    }
    private void backLoginButton_Click(){
        this.finish();
